...
2. K 线周期通过 `ktype` 整数枚举指定：1=1分钟 / 2=日(默认) / 3=周 / 4=月 / 5=年 / 6=5分钟 / 7=15分钟 / 8=30分钟 / 9=60分钟 / 10=3分钟 / 11=季 / 14=120分钟 / 15=240分钟 / 26=10分钟 / 29=180分钟
3. 复权类型通过 `autype` 指定：0=不复权 / 1=前复权(默认) / 2=后复权
4. 美股盘前盘后通过 `extended_time` 指定：0=不含(默认) / 1=含盘前盘后(仅1分钟K) / 2=含夜盘
5. 批量接口 `code_list` 最多 400 只
6. 如果用户只说股票名称没给代码，先用快照确认代码再继续
7. 返回数据时用表格或结构化格式呈现，便于阅读
8. 对于价格变动，标注涨跌幅百分比
9. 优先用 `limit`/`num` 控制返回条数，避免一次性拉取过多数据
